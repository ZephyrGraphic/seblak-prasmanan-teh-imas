...
interface OrderState {
  ...
  customerMembership: "BRONZE" | "SILVER" | "GOLD" | null;
    ...
    (set, get) => ({
      ...
      setCustomerData: (data) =>
        set({
          ...
          customerMembership: data.membership,
        }),
      ...
      addBowl: (bowl) =>
        set((state) => ({
          bowls: [
            ...state.bowls,
            { ...bowl, id: Math.random().toString(36).substring(7) },
          ],
          ...
          if (existing) {
            ...
            return {
              drinks: state.drinks.map((d) =>
                d.name === name ? { ...d, quantity: newQty } : d,
              ),
            };
          } else {
            if (delta > 0) {
              return {
                drinks: [...state.drinks, { name, price, quantity: delta }],
              };
            }
            return state;
          }
      ...
      getTotalPrice: () => {
        ...
        const drinksPrice = state.drinks.reduce(

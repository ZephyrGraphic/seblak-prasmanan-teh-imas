  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: customerName, diningOption, paymentMethod",
        },
        { status: 400 },
      );
    }
    ...
    if (settings && !settings.isOpen) {
      return NextResponse.json(
        {
          success: false,
          error: "Maaf, toko sedang tutup. Tidak dapat menerima pesanan baru.",
        },
        { status: 400 },
      );
    }
    ...
    // Map dining option
    const diningOptionEnum =
      diningOption === "Dine-in" || diningOption === "DINE_IN"
    ...
    const order = await prisma.order.create({
      data: {
        queueNumber,
        customerName,
        customerId: customerId || null, // Link to CRM customer if provided
        ...
        drinks: {
          create: drinks.map(
            (drink: { name: string; quantity: number; price: number }) => ({
              name: drink.name,
    ...
    // Update customer statistics if linked to a CRM customer
    if (customerId) {
      await prisma.customer
        .update({
          where: { id: customerId },
          data: {
            totalSpent: { increment: totalPrice || 0 },

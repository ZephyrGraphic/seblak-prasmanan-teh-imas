...
export default function RevenuePage() {
  ...
  const fetchRevenue = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/revenue?date=${dateParam}`);
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // Title
      doc.setFontSize(18);
      doc.text("Seblak Teh Imas - Daily Revenue Report", pageWidth / 2, 20, {
        align: "center",
      });
      doc.setFontSize(12);
      doc.text(`Date: ${formatDisplayDate(selectedDate)}`, pageWidth / 2, 30, {
        align: "center",
      });
      ...
      y += 7;
      doc.text(`Orders Completed: ${data.completedCount}`, 14, y);
      y += 7;
      doc.text(`Orders Pending: ${data.pendingCount}`, 14, y);
      y += 7;
      doc.text(
        `Average Ticket: Rp ${data.avgTicket.toLocaleString("id-ID")}`,
        14,
        y,
      );
      y += 7;
      doc.text(
        `Revenue Growth: ${data.revenueChange >= 0 ? "+" : ""}${data.revenueChange}%`,
        14,
        y,
      );
      y += 15;
      ...
      y += 10;
      doc.setFontSize(10);
      doc.text(`Cash: Rp ${data.cashRevenue.toLocaleString("id-ID")}`, 14, y);
      y += 7;
      doc.text(
        `Transfer (DANA/QRIS): Rp ${data.transferRevenue.toLocaleString("id-ID")}`,
        14,
        y,
      );
      y += 15;
      ...
      doc.text(
        "Note: Complete tabular data is available in the web dashboard.",
        14,
        y,
      );
      ...
      alert(
        "Gagal mengexport PDF. Terjadi kesalahan pada sistem report (Safe Mode).",
      );
    }
  };

  const maxTrendRevenue =
    data?.hourlyTrend?.reduce((max, h) => Math.max(max, h.revenue), 0) || 1;
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark pb-24">
      ...
      <div className="bg-white dark:bg-stone-900 border-b border-gray-100 dark:border-gray-800 p-4">
        ...
        {!isToday && (
          <button
            onClick={goToToday}
            className="px-3 py-1.5 text-xs font-bold rounded-full bg-gray-100 dark:bg-stone-800 text-gray-600 dark:text-gray-400"
          >
            Kembali ke Hari Ini
          </button>
        )}
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-wrap gap-3 p-4">
          <div className="flex min-w-[100%] flex-col gap-2 rounded-xl p-6 bg-primary text-white shadow-lg shadow-primary/20">
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <p className="text-white/90 text-sm font-medium">
                {(data?.revenueChange || 0) >= 0 ? "+" : ""}
                {data?.revenueChange || 0}% from previous day
              </p>
            </div>
          </div>
          ...
          <div className="flex min-w-[158px] flex-1 flex-col gap-2 rounded-xl p-5 bg-[#f5f2f0] dark:bg-stone-800">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[#181411] dark:text-stone-200 text-sm font-medium">
                Completed
              </p>
            </div>
            ...
            <p className="text-green-600 text-xs font-medium">
              {data?.pendingCount || 0} orders pending
            </p>
          </div>
          <div className="flex min-w-[158px] flex-1 flex-col gap-2 rounded-xl p-5 bg-[#f5f2f0] dark:bg-stone-800">
            <div className="flex items-center gap-2">
            ...
            <p className="text-[#181411] dark:text-white tracking-tight text-2xl font-bold">
              Rp {((data?.avgTicket || 0) / 1000).toFixed(0)}k
            </p>
          </div>
        </div>
        ...
        {/* Payment Breakdown */}
        <div>
          <h3 className="text-[#181411] dark:text-white text-lg font-bold px-4 pb-2 pt-2">
            Payment Breakdown
          </h3>
          <div className="mx-4 p-4 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-stone-900/50">
            <div className="flex justify-between items-center py-3 border-b border-gray-50 dark:border-gray-800">
              <div className="flex items-center gap-3">
                ...
                <p className="text-[#8a7260] dark:text-stone-400 text-sm font-medium">
                  Total Cash
                </p>
              </div>
              <p className="text-[#181411] dark:text-white text-base font-bold">
                Rp {(data?.cashRevenue || 0).toLocaleString("id-ID")}
              </p>
            </div>
            <div className="flex justify-between items-center py-3">
              ...
              <p className="text-[#181411] dark:text-white text-base font-bold">
                Rp {(data?.transferRevenue || 0).toLocaleString("id-ID")}
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mx-4 p-4 rounded-xl bg-[#f5f2f0] dark:bg-stone-800/50 flex items-end justify-between h-40 gap-2">
            {data?.hourlyTrend?.map((trend) => {
              const height =
                maxTrendRevenue > 0
                  ? (trend.revenue / maxTrendRevenue) * 100
                  : 10;
              return (
                <div
                  key={trend.hour}
        ...
        <div>
          <h3 className="text-[#181411] dark:text-white text-lg font-bold px-4 pb-2 pt-6">
            Most Popular Toppings
          </h3>
          <div className="px-4 pb-8 space-y-3">
            {data?.popularToppings?.map((topping, index) => (
              <div
                key={topping.name}
                className="flex items-center gap-4 bg-white dark:bg-stone-900 border border-gray-100 dark:border-gray-800 p-3 rounded-xl"
              >
                ...
                <div className="flex-1">
                  <p className="text-[#181411] dark:text-white text-sm font-bold">
                    {topping.name}
                  </p>
                  <p className="text-[#8a7260] dark:text-stone-400 text-xs">
                    Used in {topping.count} orders
                  </p>
                </div>
                <div className="text-right">
                  {index === 0 ? (
                    ...
                    <span className="text-[#8a7260] text-xs font-bold">
                      #{index + 1}
                    </span>
                  )}
                </div>

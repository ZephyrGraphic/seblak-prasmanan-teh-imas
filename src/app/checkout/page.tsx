...
export default function CheckoutPage() {
    ...
    async (phone: string) => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          setLookupMessage(
            `✓ Member ${membershipEmoji[result.data.membership]} ${result.data.membership}`,
          );
        } else {
          store.setCustomerData({
            id: null,
  ...
  const handlePhoneChange = (phone: string) => {
    store.setCustomerPhone(phone);
    // Debounce lookup
    const timer = setTimeout(() => lookupCustomer(phone), 500);
  ...
  const handleSubmitOrder = async () => {
    ...
    if (foodTotal < 10000) {
      alert(
        "Minimal pembelian seblak prasmanan adalah Rp 10.000 (tidak termasuk minuman).\nSilakan tambah topping lagi ya kak!",
      );
      return;
    }
    ...
    try {
      // Submit order to API with customerId for CRM
      ...
      store.addToHistory({
        ...
        itemsCount:
          store.bowls.length +
          store.drinks.reduce((acc, d) => acc + d.quantity, 0),
      ...
      store.clearOrder();
      router.push(
        `/receipt?queue=${encodeURIComponent(queueNumber)}&id=${order.id}`,
      );
    } catch (error) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-gray-500 mb-8">
            Belum ada seblak yang diracik nih.
          </p>
      ...
      <main className="flex-1 p-4 space-y-6">
        ...
        <div className="bg-white dark:bg-white/5 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-[#181411] dark:text-white font-bold text-lg">
              Data Pemesan
            </h3>
            {store.customerMembership && (
              <span
                className={clsx(
                  "px-3 py-1 text-xs font-bold uppercase rounded-full",
                  store.customerMembership === "GOLD"
                    ? "bg-yellow-100 text-yellow-700"
                    : store.customerMembership === "SILVER"
                      ? "bg-gray-100 text-gray-700"
                      : "bg-amber-100 text-amber-700",
                )}
              >
                {membershipEmoji[store.customerMembership]} Member{" "}
                {store.customerMembership}
              </span>
            )}
          </div>
          ...
          {/* Phone Number for Member Lookup */}
          <div className="relative">
            <Input
              placeholder="Nomor Telepon (opsional, untuk member)"
              value={store.customerPhone}
              onChange={(e) => handlePhoneChange(e.target.value)}
              type="tel"
            />
            {isLookingUp && (
              <span className="absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined animate-spin text-gray-400 text-lg">
                progress_activity
              </span>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          {store.bowls.map((bowl, idx) => (
            <div
              key={bowl.id || idx}
              className="bg-white dark:bg-white/5 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10 relative"
            >
              <button
              ...
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-[#8a7260] mt-1 space-y-0.5">
                    ...
                    <p>
                      <span className="font-bold">Kuah:</span> {bowl.kuah} •{" "}
                      {bowl.rasa}
                    </p>
                    <p>
        ...
        {/* Note */}
        <div className="bg-white dark:bg-white/5 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10">
          <h3 className="text-[#181411] dark:text-white font-bold text-sm mb-2">
            Catatan Tambahan
          </h3>
          <textarea
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-background-dark border-t border-gray-100 dark:border-white/10 z-50 max-w-[480px] mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-2xl font-extrabold text-primary">
            Rp {store.getTotalPrice().toLocaleString("id-ID")}
          </span>
        </div>
        ...
        >
          <span className="material-symbols-outlined mr-2">
            {isSubmitting ? "hourglass_empty" : "chat"}
          </span>
